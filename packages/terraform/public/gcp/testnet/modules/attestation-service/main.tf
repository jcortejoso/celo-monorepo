...
data "template_file" "template_save_variable" {
  template = "${file("${path.module}/../../../../../scripts/save_variable.sh.template")}"
}
...
data "template_file" "template_install_attestation_service" {
  count = var.attestation_service_count > 0 ? var.attestation_service_count : 0
  template = "${file("${path.module}/../../../../../scripts/install_attestation_service.sh.template")}"
  vars = {
    rid = "${count.index}",
...
resource "google_compute_instance" "attestation_service" {
  ...
  metadata_startup_script = templatefile(
    format("%s/startup.sh", path.module), {
      template_save_variable : data.template_file.template_save_variable.rendered,
      template_install_docker : data.template_file.template_install_docker.rendered,
      template_install_attestation_service : data.template_file.template_install_attestation_service[count.index].rendered,
    }
  )
}
